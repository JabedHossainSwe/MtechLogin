...
from " . dbObjectMain . "Logins inner JOIN " . dbObjectMain . "Companies companies on companies.id = " . dbObjectMain . "Logins.companyId where " . dbObjectMain . "Logins.isDeleted = 0 and (" . dbObjectMain . "Logins.email='" . $email . "' or " . dbObjectMain . "Logins.code='" . $email . "') and companies.isDeleted = 0";
    ...
    if (in_array($platForm, $expl)) {
      ...
      <script>
        document.getElementById('password').focus();
        document.getElementById('password').style.border = "1px solid red";
        document.getElementById('password_error').innerHTML = "You are not Allowed to Login From Laptop";
      </script>
      <?php
      die();
    }
    ...
    $_SESSION['lang'] = $lang;
    if ($_SESSION['id']) {
      // Insert All arabic in session////
      $sessionLanguage = array();
      $mm = "SELECT  [english]
      ,[arabic] from Mtech_translations";
      $qu = RunMain($mm);
      while ($ArabicRecords = myfetchMain($qu)) {
        array_push($sessionLanguage, ['english' => $ArabicRecords->english, 'arabic' => $ArabicRecords->arabic]);
      }
