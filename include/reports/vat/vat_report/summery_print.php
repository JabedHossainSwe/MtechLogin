...
if ($LanguageId == 1) {
	$html .= '<div id="Sale-Inovice" style="-webkit-print-color-adjust:exact !important;
		...
		<h2 style="font-size: 0.6rem ; padding-left: 5px;"> MOB:' . $getLoginUserCompanyData->mobile . ' <br> ' . $getLoginUserCompanyData->email . ' <br> VAT#: ' . $getLoginUserCompanyData->vat . '
		</h2>
		...
		<tr>
			...
			<th style="font-weight: 500;">Value</th>	
		</tr>';
	...
	while ($single = myfetch($detailsSp)) {
		$nValueTotal = $nValueTotal + $single->nValue;

		$html .= '
		<tr>
			<th style="font-weight: 300;">' . $single->Description . '</th>
			<th style="font-weight: 300;">' . AmountValue($single->nValue) . '</th>
		</tr>';
	}
	$html .= '
		<tr>
			<th style="font-weight: 600; text-align:center;" colspan="1">' . getArabicTitle('Total') . '</th>
			<th style="font-weight: 600;">' . AmountValue($nValueTotal) . '</th>
		</tr>
		</table>
		</div>';
}
...
if ($LanguageId == 2) {
	...
	<table border="1" style="width: 100%; font-size: 1rem; border-collapse: collapse;">
		<tr>
			...
			<th style="font-weight: 500;">' . getArabicTitle('Value') . '</th>	
		</tr>';
	...
	while ($single = myfetch($detailsSp)) {
		...
		$html .= '
		<tr>
			<th style="font-weight: 300;">' . getArabicTitle($single->Description) . '</th>
			<th style="font-weight: 300;">' . AmountValue($single->nValue) . '</th>
		</tr>';
	}
	$html .= '
