<?php
$mainStoreProcedure = "EXECUTE " . dbObject . "[PurchaseVatReport] @LanguageID = $lang, 
@BID = $bid, 
@PayType = $pay_type, @FromDate = '$dt', 
@ToDate = '$dt2', 
@SupplierCode = $supplier_name";
...
<div class="col-md-12 col-sm-12 col-lg-12 mb-4 text-center">
	<div class="d-grid gap-2 col-12 pl-0">
		<button style="width: 130px;" type="button" class="btn btn-w-m btn-outline-primary" id="reportPRint" onclick="PrintPopup('<?= $print ?>', 'Vat Purchase Report', 'vatPurchaseReport')"><i class="fa fa-print" aria-hidden="true"></i></button>
	</div>
</div>
<hr>
<!-- Listing -->
<table class="table table-striped table-bordered dt-responsive table-hover">
	<thead>
		<tr>
			...
			<th><span class="en">Purchase Total</span><span class="ar"><?= getArabicTitle('Purchase Total') ?></span></th>
			<th><span class="en">P.Discount</span><span class="ar"><?= getArabicTitle('P.Discount') ?></span></th>
			<th><span class="en">P.Net Total</span><span class="ar"><?= getArabicTitle('P.Net Total') ?></span></th>
			<th><span class="en">P.Vat Total</span><span class="ar"><?= getArabicTitle('P.Vat Total') ?></span></th>
			...
			<th><span class="en">Grand Total</span><span class="ar"><?= getArabicTitle('Grand Total') ?></span></th>
			<th><span class="en">Net Purchase Amount</span><span class="ar"><?= getArabicTitle('Net Purchase Amount') ?></span></th>
	...
	<tbody>
		...
		if ($pages->items_total > 0) {
			...
			while ($single  =   myfetch($result)) {
				...
				<td><span class="text-warp"><?php echo AmountValue($single->NetPurchaseAmount); ?> </span></td>
