<?php
$mainStoreProcedure = "EXEC  " . dbObject . "GetSalesProfitCal @bid='" . $bid . "',@SpType='" . $SpType . "',@dt='" . $dt . "',@dt2='" . $dt2 . "',@CustSupId='" . $CustSupId . "',@UsrId='" . $UsrId . "',@LanguageId='" . $LanguageId . "'";
...
<div class="col-md-12 col-sm-12 col-lg-12 mb-4 text-center">
	<div class="d-grid gap-2 col-12 pl-0">
		<button style="width: 130px;" type="button" class="btn btn-w-m btn-outline-primary" id="reportPRint" onclick="PrintPopup('<?= $print ?>', 'Sale Profit Calculator General', 'generalPrint')"><i class="fa fa-print" aria-hidden="true"></i></button>
	</div>
</div>
<hr>
<!-- Listing -->
<table class="table table-striped table-bordered dt-responsive table-hover ">
	...
	if ($pages->items_total > 0) {
		...
		while ($single  =   myfetch($result)) {
			...
			<td><span class="text-warp"><?php echo $single->Type; ?> </span></td>
