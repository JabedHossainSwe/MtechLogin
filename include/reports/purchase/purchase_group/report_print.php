...
if ($LanguageId == 1) {
	...
	$html .= '<div id="Sale-Inovice" style="-webkit-print-color-adjust:exact !important;
		...
		<tr>
			...
			<th style="font-weight: 500;">PurRetTotal</th>
			<th style="font-weight: 500;">NetPurchase</th>
			<th style="font-weight: 500;">VatTotal</th>
			<th style="font-weight: 500;">VatTotalPurRet</th>
	...
	while ($single = myfetch($detailsSp)) {
		...
		$PurRetTotalTot = $PurRetTotalTot + $single->PurRetTotal;
		$NetPurchaseTot = $NetPurchaseTot + $single->NetPurchase;
		$VatTotalTot = $VatTotalTot + $single->VatTotal;
		$VatTotalPurRetTot = $VatTotalPurRetTot + $single->VatTotalPurRet;
		...
		<tr>
			...
			<th style="font-weight: 300;">' . AmountValue($single->PurRetTotal) . '</th>
			<th style="font-weight: 300;">' . AmountValue($single->NetPurchase) . '</th>
			<th style="font-weight: 300;">' . AmountValue($single->VatTotal) . '</th>
			<th style="font-weight: 300;">' . AmountValue($single->VatTotalPurRet) . '</th>
	...
	$html .= '
		<tr>
			...
			<th style="font-weight: 600;">' . AmountValue($PurRetTotalTot) . '</th>
			<th style="font-weight: 600;">' . AmountValue($NetPurchaseTot) . '</th>
			<th style="font-weight: 600;">' . AmountValue($VatTotalTot) . '</th>
			<th style="font-weight: 600;">' . AmountValue($VatTotalPurRetTot) . '</th>
...
if ($LanguageId == 2) {
	...
	<table border="1" style="width: 100%; font-size: 1rem; border-collapse: collapse;">
		<tr>
			...
			<th style="font-weight: 500;">' . getArabicTitle('PurRetTotal') . '</th>
			<th style="font-weight: 500;">' . getArabicTitle('NetPurchase') . '</th>
			<th style="font-weight: 500;">' . getArabicTitle('VatTotal') . '</th>
			<th style="font-weight: 500;">' . getArabicTitle('VatTotalPurRet') . '</th>
	...
	$html .= '
	<tr>
		<th style="font-weight: 600; text-align:center;" colspan="1">'.getArabicTitle('Total').'</th>
		<th style="font-weight: 600;">' . AmountValue($NetTotalTot) . '</th>
		<th style="font-weight: 600;">' . AmountValue($PurRetTotalTot) . '</th>
		<th style="font-weight: 600;">' . AmountValue($NetPurchaseTot) . '</th>
		<th style="font-weight: 600;">' . AmountValue($VatTotalTot) . '</th>
