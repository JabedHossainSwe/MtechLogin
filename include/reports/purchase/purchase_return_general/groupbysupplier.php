...
$mainStoreProcedure = "EXEC  " . dbObject . "GetPurReturnGen @bid='" . $bid . "',@GroupByType='" . $GroupByType . "',@SpType='" . $SpType . "',@FBillno='" . $FBillno . "',@TBillno='" . $TBillno . "',@dt='" . $dt . "',@dt2='" . $dt2 . "',@CustSupId='" . $CustSupId . "',@UsrId='" . $UsrId . "',@LanguageId='" . $LanguageId . "',@OrderBy='" . $OrderBy . "',@PurType ='" . $PurType . "',@IsNoVatOnInvoice ='" . $IsNoVatOnInvoice . "'";
...
<div class="col-md-12 col-sm-12 col-lg-12 mb-4 text-center">
	<div class="d-grid gap-2 col-12 pl-0">
		<button style="width: 130px;" type="button" class="btn btn-w-m btn-outline-primary" id="reportPRint" onclick="PrintPopup('<?= $print ?>', 'Purchase Return General', 'purchaseReturnSupplier')"><i class="fa fa-print" aria-hidden="true"></i></button>
	</div>
</div>
	...
	<thead>
		<tr>
			...
			<th><span class="en">Total Vat</span><span class="ar"><?= getArabicTitle('Total Vat') ?></span> </th>
			<!-- <th><span class="en">AdvTax</span><span class="ar"><?= getArabicTitle('AdvTax') ?></span></th> -->
			<!-- <th><span class="en">TotalTax</span><span class="ar"><?= getArabicTitle('TotalTax') ?></span></th> -->
			<!-- <th><span class="en">DatTotal</span><span class="ar"><?= getArabicTitle('DatTotal') ?></span> </th> -->
			<th><span class="en">GrandTotal</span><span class="ar"><?= getArabicTitle('GrandTotal') ?></span></th>
			<!-- <th><span class="en">WHTax</span><span class="ar"><?= getArabicTitle('WHTax') ?></span> </th> -->
	...
	<tbody>
		...
		if ($pages->items_total > 0) {
			...
			while ($single  =   myfetch($result)) {
				...
				<!-- <td><span class="text-warp"><?php echo AmountValue($single->TotalTax); ?> </span></td> -->
				<!-- <td><span class="text-warp"><?php echo AmountValue($single->DatTotal); ?> </span></td> -->
				<td><span class="text-warp"><?php echo AmountValue($single->GrandTotal); ?> </span></td>
				<!-- <td><span class="text-warp"><?php echo AmountValue($single->WHTax); ?> </span></td> -->
