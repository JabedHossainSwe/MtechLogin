...
function loadlisting() {
  ...
  $.post("include/sup_group/loadlisting.php", {}, function (data) {
...
function Add() {
  ...
  if(lang == 1){
  document.getElementById('modalTitle').innerHTML = "Supplier Group";
  }
  else{
  document.getElementById('modalTitle').innerHTML = "مجموعة الموردين";
  }
  document.getElementById("helperText").innerHTML = "";
  javascript: $("#myModal2").modal("show", { backdrop: "static" });
  $.post("include/sup_group/Add.php", {}, function (data) {
...
function edit(id) {
  ...
  javascript: $("#myModal2").modal("show", { backdrop: "static" });
  $.post("include/sup_group/edit.php", { id: id }, function (data) {
    $("#modalContent").html(data);
  });
}
...
function save() {
  ...
  $("#sup_group_add_btn").attr("disabled", true);
  $.ajax({
    url: "include/sup_group/save.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
    ...
    success: function (dataofconfirm) {
      ...
      $("#sup_group_add_btn").attr("disabled", false);
    },
  });
}
...
function update() {
  ...
  var myform = document.getElementById("save_form");
  var fd = new FormData(myform);
  $("#sup_group_update_btn").attr("disabled", true);
  $.ajax({
    url: "include/sup_group/update.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
    ...
    success: function (dataofconfirm) {
      $("#save").html(dataofconfirm);
      $("#sup_group_update_btn").attr("disabled", false);
    },
  });
}
...
function deleteEntry(id) {
  ...
  if (result) {
    document.getElementById("deleteEntry").innerHTML =
      "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
    $.post("include/sup_group/deleteEntry.php", { id: id }, function (data) {
