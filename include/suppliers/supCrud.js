...
function saveSupplier() {
  ...
  $.ajax({
    url: "include/suppliers/save.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function updateSupplier() {
  ...
  var fd = new FormData(myform);
  $.ajax({
    url: "include/suppliers/update.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function deleteEntry(CCode, bid) {
  ...
  if (result) {
    document.getElementById("deleteEntry").innerHTML =
      "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
    $.post(
      "include/suppliers/deleteEntry.php",
      { CCode: CCode, bid: bid },
      function (data) {
...
function reloadVoucher() {
  ...
  if (anyaction) {
    ...
    $.post(
      "include/suppliers/reloadVoucher.php",
      {
        Bid: Bid,
        CCode: CCode,
      },
...
function editVoucher(Bid, CCode) {
  ...
  $.post(
    "include/suppliers/editVoucher.php",
    {
      Bid: Bid,
      CCode: CCode,
    },
