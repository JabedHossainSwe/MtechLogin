...
function updateRow(idx) {
  ...
  $.post(
    ...
    function (data) {
      ...
      $("#action_id").html(
        '<button type="button" name="add_row" id="add_row" class="btn btn-info" onclick="addRow()" style="display:none;">Add</button>'
      );
    }
  );
}
...
function SaveSalesVoucher() {
  ...
  document.getElementById("SalesVoucherDiv").innerHTML =
    "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
  $.ajax({
    url: "include/sales/SaveSalesVoucher.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function product_add_addRow(Bid, Billno, sBid, LanguageId) {
  var in_html = $("#row_append").html();
  $.post(
    "vouchers/sales/sections/product_add_addRow.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function saveFinalStep(SBBillno, Bid) {
  $.post(
    "include/sales/saveFinalStep.php",
    {
      SBBillno: SBBillno,
      Bid: Bid,
    },
