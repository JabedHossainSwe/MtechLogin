...
$ff = "Exec " . dbObject . "SPOpenQuantitySelectWeb  @SrchBy=1,@Billno=$Billno,@Bid=$Bid,@sBid=$sbid,@LanguageId=$LanguageId,@FRecNo=0,@ToRecNo=1";
...
if ($Billno == '') {
  ...
  $sale_invoice = getOpeningQuantityemplateNew($GotBill, $Bid, $LanguageId);
  ...
  //$mpdf->Output();
  $mpdf->Output('Opening Quantity Invoice-' . $language . '-' . $Billno . '.pdf', 'D');
?>
  <script>
    $('#printInvoice').html();
  </script>
