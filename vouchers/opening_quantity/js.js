...
function addRow() {
  ...
  if (oldQty == "") {
    document.getElementById("oldQty").style.border = "1px solid red";
    $("#oldQty").focus();
    return false;
  }

  if (newQty == "") {
    document.getElementById("newQty").style.border = "1px solid red";
    $("#newQty").focus();
    return false;
  }
  ...
  $.post(
    "vouchers/opening_quantity/addRow.php",
    {
      Pcode: code,
...
function edit_row(idx) {
  ...
  var oldQty = $("#oldQty" + idx).val();
  var newQty = $("#newQty" + idx).val();
...
function updateRow(idx) {
  ...
  $.post(
    "vouchers/opening_quantity/updateRow.php",
    {
      Pcode: code,
      product: product_name,
      unit_id: unit,
...
function getProductUnit(code) {
  ...
  $.post(
    "vouchers/opening_quantity/getProductUnit.php",
    {
      code: code,
    },
    function (data) {
...
function salesAddRowCalculations() {
  var oldQty = $("#oldQty").val();
  var price = $("#price").val();
  var total = parseFloat(oldQty) * parseFloat(price);
  $("#total").val(total);
...
function SaveQuantityVoucher() {
  ...
  document.getElementById("SalesVoucherDiv").innerHTML =
    "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
  $.ajax({
    url: "vouchers/opening_quantity/SaveQuantityVoucher.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function UpdateQuantityVoucher() {
  ...
  document.getElementById("SalesVoucherDiv").innerHTML =
    "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
  $.ajax({
    url: "vouchers/opening_quantity/UpdateQuantityVoucher.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function fetchProductDetailsFromCode(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/opening_quantity/fetchProductDetailsFromCode.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function fetchProductUnits(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/opening_quantity/fetchProductUnits.php",
      {
        code: vvl,
        Bid: Bid,
      },
...
function setProductUnits(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/opening_quantity/setProductUnits.php",
      {
        code: vvl,
        Bid: Bid,
      },
...
function fetchProductDetails(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/opening_quantity/fetchProductDetails.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function Pricecalculations(vvl, tp) {
  ...
  // if (vatPer != "0") {
  //   $.post(
  //     "vouchers/opening_quantity/Pricecalculations.php",
  //     {
  //       vvl: vvl,
  //       tp: tp,
  //       oldQty: oldQty,
...
function LoadUnitPrice(Uid, vvl) {
  if (vvl != "" && Uid != "") {
    ...
    document.getElementById("fetchProductDetails").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "vouchers/opening_quantity/LoadUnitPrice.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function getProductList(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/opening_quantity/getProductList.php",
      {
        code: vvl,
      },
      function (data) {
...
function CheckSaleBill(sale_bill_no) {
  ...
  document.getElementById("CheckSaleBill").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/opening_quantity/CheckSaleBill.php",
    {
      Bid: Bid,
      Billno: sale_bill_no,
    },
...
function product_add_addRow(Bid, Billno, sBid, LanguageId) {
  var in_html = $("#row_append").html();
  $.post(
    "vouchers/opening_quantity/product_add_addRow.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function bank_section(Bid, Billno, sBid, LanguageId) {
  ...
  $.post(
    "vouchers/opening_quantity/sections/bank_section.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function loadBanks(vvl) {
  if (vvl != "") {
    document.getElementById("cashCreditOption").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "vouchers/opening_quantity/loadBanks.php",
      {
        code: vvl,
      },
      function (data) {
...
function sale_bill_no_area(Bid, Billno, sBid, LanguageId) {
  document.getElementById("sale_bill_no_area").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/opening_quantity/sections/sale_bill_no_area.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
//   $.post(
//     "vouchers/opening_quantity/saveFinalStep.php",
//     {
//       SBBillno: SBBillno,
//       Bid: Bid,
//     },
...
function customerValidation(e) {
  ...
  if (anyaction) {
    ...
    //   "<img width='80px' src='loader/wheel.gif'/>";
    $.ajax({
      url: "vouchers/opening_quantity/updateSalesForm.php",
      data: fd,
      cache: false,
      processData: false,
      contentType: false,
...
function reloadVoucher() {
  ...
  if (anyaction) {
    ...
    $.post(
      "vouchers/opening_quantity/reloadVoucher.php",
      {
        Bid: Bid,
        Billno: Billno,
      },
...
function editVoucher(type) {

  if(type == 'prv'){
    var Billno = $("#prv_billno").val();
  }
  else{
    var Billno = $("#nxt_billno").val();
  }
  var Bid = $("#Bid").val();

  document.getElementById("editVoucher").innerHTML="<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/opening_quantity/editVoucher.php",
    {
      Bid: Bid,
      Billno: Billno,
    },
...
function deleteOpeningQuantity(Billno, Bid, sBid) {
  document.getElementById("editVoucher").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";

  $.post(
    "vouchers/opening_quantity/deleteOpeningQuantity.php",
    {
      Billno: Billno,
      Bid: Bid,
      sBid: sBid,
...
function Print_report_details()
{
  ...
  window.open(
    'vouchers/opening_quantity/print.php?Billno='+bill_no+'&Bid='+Bid+'&LanguageId='+print_language+'',
    '_blank' // <- This is what makes it open in a new window.
    );
}
...
$.post("vouchers/opening_quantity/send_email.php",{
b_id:b_id,
bill_id:bill_id,
LanguageId:LanguageId,
email:email,
...
function getBillNo(Bid) {
  $.post(
    "vouchers/opening_quantity/getBillNo.php",
    {
      Bid: Bid
    },
    function (data) {
