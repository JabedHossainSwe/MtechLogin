...
$QueryGet = Run("Select *,[Discount%] as disper from " . dbObject . "DataOut where Billno = $Billno and Bid = $Bid  and sbid=$sBid");
$billData = myfetch($QueryGet);
// print_r($billData);
?>

<form action="javascript:UpdateSalesVoucher()" id="sales_report_form" method="post" id="sales_voucher" class="ibox-content  filter_container">
            ...
            <div class="col-6">
              <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <?php
                  $qt = "select BillNo from dataout where BillNo = (select max(BillNo) from dataout where BillNo < '" . $Billno . "' and Bid='" . $Bid . "' and isDeleted = 0) and Bid = '" . $Bid . "' and isDeleted = 0";
                  $previousQuery = Run($qt);
                  $getPreviousId = myfetch($previousQuery)->BillNo;
                  if ($getPreviousId != '') {
                  ?>
  
                    <button type="button" class="btn btn-success" onclick="editVoucher('<?= $Bid ?>','<?= $getPreviousId ?>','<?= $sBid ?>')"><i class="fa fa-backward"></i></i></button>
                  <?php
                  }
                  $qt = "select BillNo from dataout where BillNo = (select min(BillNo) from dataout where BillNo > '" . $Billno . "' and Bid='" . $Bid . "' and isDeleted = 0) and Bid = '" . $Bid . "' and isDeleted = 0";
                  $nextQuery = Run($qt);
                  ...
                  ?>
                    ...
                    <button type="button" class="btn btn-success" onclick="editVoucher('<?= $Bid ?>','<?= $getNextId ?>','<?= $sBid ?>'))"><i class="fa fa-forward"></i></i></button>
  
                  <?php } ?>
                  <button type="button" class="btn btn-success" onclick="deleteSale('<?= $Billno ?>', '<?= $Bid ?>', '<?= $sBid ?>')"><i class="fa fa-trash"></i></button>
          ...
          <div class="row">
              ...
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-total_int">
                    <input id="f_total" name="f_total" type="text" readonly value="<?= round($billData->Total, 2) ?>" class="form-control">
                  </div>
                </div>
              </div>
            </div>
            <div class="col-lg-2">
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-group">
                    <input id="fdisPer" name="fdisPer" type="text" class="form-control" value="<?= $billData->disper ?>" onkeyup="calculateWholeDiscountAmount(this.value)">
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="col-lg-2">
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-group">
                    <input id="totVat" name="totVat" readonly type="text" value="<?= round($billData->totalVat, 2) ?>" class="form-control">
                  </div>
                </div>
              </div>
            </div>
            <div class="col-lg-2">
              <div class="row">
                ...
                <div class="col-md-7">
                  <div class="form-group">
                    <input value="<?= round($billData->vatPTotal, 2) ?>" id="grandTotal" name="grandTotal" readonly type="text" class="form-control">
                  </div>
                </div>
              </div>
            </div>
