...
while ($counter <= $row_count) {
  ...
  $Discount = !empty($disAmt) ? $disAmt : '0';

  // $Total = $Price * $qty;
  // $Discount = 0;

  $netT = $_POST['netTotal' . $counter];
...
$saleSp = "exec " . dbObject . "InsertSaleUpdateWeb
@Bid=$Bid
,@BillNo=$BillNo
,@sbBillno='" . $sbBillno . "'
,@CSID=$CSID
...
// $filename = $PNG_WEB_DIR.'Sale-'.$SBBillno.'-'.$Bid.'-'.$_SESSION['companyId'].'.png';
// file_put_contents($filename, file_get_contents($displayQRCodeAsBase64));
...
// $PNG_WEB_DIR = '../../../MtechMobile/includes/sales/QrCodes/';
// $filename = $PNG_WEB_DIR.'Sale-'.$SBBillno.'-'.$Bid.'-'.$_SESSION['companyId'].'.png';
// file_put_contents($filename, file_get_contents($displayQRCodeAsBase64));

$PNG_TEMP_DIR = '../../../MtechMobile/includes/sales/QrCodes/';
$filename = $PNG_TEMP_DIR . 'Sale-' . $SBBillno . '-' . $Bid . '-' . $_SESSION['companyId'] . '.png';
file_put_contents($filename, file_get_contents($displayQRCodeAsBase64));
...
if ($execute) { ?>
  ...
  <script>
    ...
    reloadVoucherAgainstBill('<?= $Bid ?>', '<?= $SBBillno ?>','<?=$sbid?>');
  </script>
<?php }
?>