...
<?php
$row_count = 1;
$tt = "EXECUTE " . dbObject . "SPDataDeliveryDetailSelectWeb @SrchBy=0, @Billno=$Billno,@Bid=$Bid,@sBid=$sBid,@LanguageId=1,@FRecNo=0,@ToRecNo=100";
$detailsSp = Run($tt);
while ($getDetails = myfetch($detailsSp)) {
	...
	$vatPer = 0;
	$vatTotal = 0;
	$vatPTotal = round($getDetails->Price, 2);
