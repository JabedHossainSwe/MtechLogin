...
while ($getDetails = myfetch($detailsSp)) {
	...
	echo $abx = "Exec ".dbObject."GetVatValueFromSalPrice @vatPer=$vatPer,@SPrice=$sprice";
	$NewSp = Run($abx);
	$getV = myfetch($NewSp);
...
<tr id="row_<?php echo $row_count ?>">
    ...
    <td>
        <input type="hidden" name="vatSprice<?php echo $row_count ?>" class="t_vatSprice" id="vatSprice<?php echo $row_count ?>" value="<?php echo $vatSprice + $vatAmt ?>"><?php echo $vatSprice + $vatAmt ?>
