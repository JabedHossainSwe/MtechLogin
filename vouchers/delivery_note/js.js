...
function addRow() {
  ...
  $.post(
    "vouchers/delivery_note/addRow.php",
    {
      Pcode: code,
...
function updateRow(idx) {
  ...
  $.post(
    "vouchers/delivery_note/updateRow.php",
    {
      Pcode: code,
      product: product_name,
      unit_id: unit,
...
function getProductUnit(code) {
  ...
  $.post(
    "vouchers/delivery_note/getProductUnit.php",
    {
      code: code,
    },
    function (data) {
...
function SaveDeliverNote() {
  ...
  if (ftotal == "" || parseFloat(ftotal) == 0) {
    $("#ftotal").css("border", "2px solid red");
    anyaction = true;
  } else {
    $("#ftotal").css("border", "1px solid green");
  }
  ...
  document.getElementById("SalesVoucherDiv").innerHTML =
    "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
  $.ajax({
    url: "vouchers/delivery_note/SaveDeliverNote.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function UpdateDeliveryNote() {
  ...
  document.getElementById("SalesVoucherDiv").innerHTML =
    "<div style='background: #fff;text-align:center'><img width='80px' src='assets/img/loaders/loader.gif'/><div>";
  $.ajax({
    url: "vouchers/delivery_note/UpdateDeliveryNote.php",
    data: fd,
    cache: false,
    processData: false,
    contentType: false,
...
function fetchProductDetailsFromCode(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/delivery_note/fetchProductDetailsFromCode.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function fetchProductUnits(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/delivery_note/fetchProductUnits.php",
      {
        code: vvl,
        Bid: Bid,
      },
...
function setProductUnits(vvl, uid) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/delivery_note/setProductUnits.php",
      {
        code: vvl,
        Bid: Bid,
        uid: uid,
...
function fetchProductDetails(vvl, Uid) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/delivery_note/fetchProductDetails.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function Pricecalculations(vvl, tp) {
  ...
  // if (vatPer != "0") {
  //   $.post(
  //     "vouchers/delivery_note/Pricecalculations.php",
  //     {
  //       vvl: vvl,
  //       tp: tp,
  //       qty: qty,
...
function calculateSingleVatTotal(vvl) {
  ...
  var total = parseFloat(Sprice) * parseFloat(qty);
  $("#total").val(total);
  $("#netTotal").val(total);
}
...
function LoadUnitPrice(Uid, vvl) {
  if (vvl != "" && Uid != "") {
    ...
    document.getElementById("fetchProductDetails").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "vouchers/delivery_note/LoadUnitPrice.php",
      {
        code: vvl,
        Bid: Bid,
        Uid: Uid,
...
function getProductList(vvl) {
  if (vvl != "") {
    ...
    $.post(
      "vouchers/delivery_note/getProductList.php",
      {
        code: vvl,
      },
      function (data) {
...
function salesTotalCalculation() {
  var tot_Sprice = 0;
  $(".tot_total").each(function () {
    tot_Sprice += +$(this).val();
  });
  tot_Sprice = tot_Sprice.toFixed(2);

  var t_totMt = 0;
  $(".tot_netTotal").each(function () {
    t_totMt += +$(this).val();
  });
  t_totMt = t_totMt.toFixed(2);

  $("#ftotal").val(tot_Sprice);

  $("#fnetTotal").val(t_totMt);
...
function CheckSaleBill(sale_bill_no) {
  ...
  document.getElementById("CheckSaleBill").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/delivery_note/CheckSaleBill.php",
    {
      Bid: Bid,
      Billno: sale_bill_no,
    },
...
function product_add_addRow(Bid, Billno, sBid, LanguageId) {
  var in_html = $("#row_append").html();
  $.post(
    "vouchers/delivery_note/sections/product_add_addRow.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function bank_section(Bid, Billno, sBid, LanguageId) {
  ...
  $.post(
    "vouchers/delivery_note/sections/bank_section.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function loadBanks(vvl) {
  if (vvl != "") {
    document.getElementById("cashCreditOption").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "vouchers/delivery_note/loadBanks.php",
      {
        code: vvl,
      },
      function (data) {
...
function sale_bill_no_area(Bid, Billno, sBid, LanguageId) {
  document.getElementById("sale_bill_no_area").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/delivery_note/sections/sale_bill_no_area.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function saveFinalStep(SBBillno, Bid) {
  $.post(
    "vouchers/delivery_note/saveFinalStep.php",
    {
      SBBillno: SBBillno,
      Bid: Bid,
    },
...
function customerValidation(e) {
  ...
  if (anyaction) {
    ...
    //   "<img width='80px' src='loader/wheel.gif'/>";
    $.ajax({
      url: "vouchers/delivery_note/updateSalesForm.php",
      data: fd,
      cache: false,
      processData: false,
      contentType: false,
...
function reloadVoucher() {
  ...
  if (anyaction) {
    ...
    $.post(
      "vouchers/delivery_note/reloadVoucher.php",
      {
        Bid: Bid,
        Billno: Billno,
      },
...
function CheckQuotationBill() {
  ...
  if (anyaction) {
    ...
    $.post(
      "vouchers/delivery_note/CheckQuotationBill.php",
      {
        Bid: Bid,
        Billno: Billno,
      },
...
function editVoucher(Bid, Billno) {
  ...
  $.post(
    "vouchers/delivery_note/editVoucher.php",
    {
      Bid: Bid,
      Billno: Billno,
    },
...
function quotation_bill_no_area(Bid, Billno, sBid, LanguageId) {
  document.getElementById("bill_no_area").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/delivery_note/sections/quotation_bill_no_area.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function product_add_addRow(Bid, Billno, sBid, LanguageId) {
  var in_html = $("#row_append").html();
  $.post(
    "vouchers/delivery_note/product_add_addRow.php",
    {
      Bid: Bid,
      Billno: Billno,
      sBid: sBid,
...
function deleteDeliveryNote(Billno, Bid, sBid) {
  document.getElementById("editVoucher").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";

  $.post(
    "vouchers/delivery_note/deleteDeliveryNote.php",
    {
      Billno: Billno,
      Bid: Bid,
      sBid: sBid,
...
function Print_report_details() {
  ...
  window.open(
    "vouchers/delivery_note/print.php?Billno=" +
      bill_no +
      "&Bid=" +
      Bid +
      "&LanguageId=" +
...
function sendemailVoucher() {
  ...
  $.post(
    "vouchers/delivery_note/send_email.php",
    {
      b_id: b_id,
      bill_id: bill_id,
      LanguageId: LanguageId,
...
function loadBanksagainstBrank(bnkVal) {
  if (bnkVal != "") {
    document.getElementById("loadBanksagainstBrank").innerHTML =
      "<img width='80px' src='loader/wheel.gif'/>";
    $.post(
      "vouchers/delivery_note/loadBanksagainstBrank.php",
      {
        code: bnkVal,
      },
      function (data) {
...
function loadBanksagainstBranchWithBill(Bid, BillNo) {
  document.getElementById("loadBanksagainstBrank").innerHTML =
    "<img width='80px' src='loader/wheel.gif'/>";
  $.post(
    "vouchers/delivery_note/sections/loadBanksagainstBranchWithBill.php",
    {
      Bid: Bid,
      BillNo: BillNo,
    },
...
function loadCustArea() {
  var cid = document.getElementById("customer_id").value;
  $.post(
    "vouchers/delivery_note/loadCustArea.php",
    {
      cid: cid,
    },
    function (data) {
