...
$ff = "Exec " . dbObject . "SPDataDeliverySelectWeb @SrchBy=1,@Billno=$Billno,@Bid=$Bid,@sBid=$sbid,@LanguageId=$LanguageId,@FRecNo=1,@ToRecNo=100";
...
if ($Billno == '') {
  ...
  $sale_invoice = getDNPrintTemplateNew($GotBill, $Bid, $LanguageId);
  ...
  //$mpdf->Output();
  $mpdf->Output('Delivery Note Invoice-' . $language . '-' . $Billno . '.pdf', 'D');
