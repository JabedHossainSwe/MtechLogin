...
while ($counter <= $row_count) {
  ...
  $vatPTotalRow = $_POST['grand_total' . $counter];
  $vatPTotalRow = !empty($vatPTotalRow) ? $vatPTotalRow : '0';
  ...
  $level3 = $_POST['level3' . $counter];
  $level3 = !empty($level3) ? $level3 : '0';
  ...
  if ($PCode != '0' && $PCode != '') {
    ...
    $delimeter = 'µ';
    $GetQtyInLow = "dbo.GetQtyInLow(" . $Uid . "," . ($Qty + $bonusQTyValue1) . ")";

    $GetQtyInLow = !empty($GetQtyInLow) ? $GetQtyInLow : '';
    ...
    $currentRow = $CSID . "," . $Bid . "," . $Pid . "," . $Uid . "," . $Qty . "," . $GetQtyInLow. "," . $bonus . "," . $QtyInLowQtyBOnus  . "," . $price . "," . $total . "," . $disPer . "," . $disAmt . "," . $net_total . "," . $vatPer . "," . $vatAmt . "," . $vattotal . "," . $vatPTotalRow . "," . $cpp . "," . $level3 . "," . $SPrice . "," . $LSPrice  . "," . $altCode . "," . $EmpID . "," . $ResEmpID . "," . $counter;
...
$qry = "EXECUTE[dbo].[InsertDataInReturnWeb]	@BillDate='$billDate'	
	,@Bid=$Bid
	,@CSID=$CSID	
	,@SPType=$SPType
	,@Total=$f_total_int
...
$filename = $PNG_TEMP_DIR . 'PurchaseReturn-' . $SBBillno . '-' . $Bid . '-' . $_SESSION['companyId'] . '.png';
