...
<div class="content">
	<h2 class="heading_fixed"><span class="en">Sales Return</span><span class="ar"><?= getArabicTitle('Sales Return') ?></span></h2>
</div>
<div class="content_form">
	<div class="form_list">
		<div class="mb-3">
			<span class="en">Grand Total :</span><span id="payment_screen_total"><span class="ar">: <?= getArabicTitle('Grand Total') ?></span>
		</div>
		<div class="mb-3" style="display: none;">
			<span class="en">Remaining :</span><span id="payment_screen_remaining"><span class="ar">: <?= getArabicTitle('Remaining') ?></span>
		</div>
		<div class="row mb-3">
...
<input type="button" onClick="bank_section('<?=$Bid?>','<?=$Billno?>','<?=$sBid?>',<?=$LanguageId?>);	" value="reload"><?php */ ?>
	...
	Where b.id=d.paytype  and billno='" . $Billno . "' and d.bid='" . $Bid . "' and d.sbid='" . $sBid . "' and b.mbid='" . $Bid . "' and b.id='" . $getBranches->id . "'   order by d.id");
		...
		<div class="mb-3 overflow-hidden">
			...
			<button type="button" class="btn btn-success btn-actions float-end submit-next" onclick="customerValidation(this);">Finish & Save <i class="fa fa-arrow-right icon-font"></i>
			</button>
		</div>
	</div>
</div>
