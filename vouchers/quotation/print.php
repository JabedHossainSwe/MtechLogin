...
$LanguageId = !empty($LanguageId) ? $LanguageId: '1';
  $ff = "Exec ".dbObject."SPSalQuotationSelectWeb @SrchBy=1,@Billno=$Billno,@Bid=$Bid,@sBid=1,@LanguageId=$LanguageId,@FRecNo=0,@ToRecNo=100";
...
else
{
  ...
  //$sale_invoice = getSalePrintTemplate($Billno,$Bid,$LanguageId);
   ...
   $sale_invoice = getQuotationPrintTemplate($GotBill,$Bid,$LanguageId);
  //  die();
...
//$mpdf->Output();
$mpdf->Output('Quotation Invoice-'.$language.'-'.$Billno.'.pdf', 'D');	
