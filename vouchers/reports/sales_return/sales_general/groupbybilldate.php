<?php
   ...
   $qqst = "SELECT count(Dataout.billdate) as tol
...
$condition
group by CAST(CONVERT(VARCHAR(10),Dataout.billdate,121) AS DATETIME), DataOut.BID,Branch.BName 
order by DataOut.BID ASC";	
		$sql_forms = Run($qqst);
...
sum(ISNULL(DataOut.GProfit,0)) as GProfit,
...
0 as BillNo,  CAST(CONVERT(VARCHAR(10),Dataout.BillDate,121) AS DATETIME) as BillDate,
sum(ISNULL(DataOut.Total,0)) as Total,sum(ISNULL(DataOut.Discount,0)) as Discount,
sum(ISNULL(DataOut.NetTotal,0)) as NetTotal, null AS Customer,0 AS CSID,sum(ISNULL(DataOut.totalCost,0)) as totalCost,
sum(ISNULL(DataOut.NProfit,0)) as NProfit,Branch.BName BranchName 
FROM Branch INNER JOIN DataOut ON Branch.Bid = DataOut.Bid LEFT OUTER JOIN
Emp ON DataOut.EmpID = Emp.Cid LEFT OUTER JOIN CustFile ON DataOut.CSID = CustFile.Cid 
$condition
group by CAST(CONVERT(VARCHAR(10),Dataout.billdate,121) AS DATETIME), DataOut.BID,Branch.BName 
